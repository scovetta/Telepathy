...
if ($Configuration -eq 'Debug') {
    ...
    Copy-Item "$env:REPO_ROOT\src\soa\Echoclient\bin\Release" -Destination "$env:REPO_ROOT\artifacts\Release\Echoclient" -Recurse	
    Copy-Item "$env:REPO_ROOT\deploy\DeployScript" -Destination "$env:REPO_ROOT\artifacts\Release\DeployScript" -Recurse
}