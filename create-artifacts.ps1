...
if ($Configuration -eq 'Debug') {
    ...
    Copy-Item "$env:REPO_ROOT\deploy\EnableTelepathyStorage.ps1" -Destination "$env:REPO_ROOT\artifacts\Release\EnableTelepathyStorage.ps1"
    Copy-Item "$env:REPO_ROOT\deploy\StartSessionLauncher.ps1" -Destination "$env:REPO_ROOT\artifacts\Release\StartSessionLauncher.ps1"
}