...
namespace Microsoft.Hpc.Scheduler.Session.Internal.SessionLauncher
{
    ...
    class SessionLauncher : DisposableObject, ISessionLauncher
    {
        ...
        /// </summary>
        private const int ConnectToSchedulerTimeout = int.MaxValue; // 9 * 60 * 1000;
       ...
       // private const string RegistryPathEnv = "CCP_SERVICEREGISTRATION_PATH";
        ...
        /// </summary>
        private const string TaskCommandLine64 = @"HpcServiceHost.exe";

        /// <summary>
        /// the commandline for the service task.
        /// note: no %CCP_HOME% on azure
        /// </summary>
        private const string TaskCommandLine32 = "HpcServiceHost32.exe";
        ...
        /// </summary>
        private Dictionary<string, SessionPool> durableSessionPool = new Dictionary<string, SessionPool>();
        ...
        /// Value: Session Id list
        /// </summary>
        private Dictionary<string, SessionPool> nonDurableSessionPool = new Dictionary<string, SessionPool>();
        ...
        public SessionLauncher(string headNode, bool runningLocal, BrokerNodesManager brokerNodesManager)
        {
            ...
            do
            {
                ...
                {
                    if (stopWatch.ElapsedMilliseconds > SessionLauncher.ConnectToSchedulerTimeout)
                    {
        ...
        async Task<SessionAllocateInfoContract> ISessionLauncher.AllocateV5Async(SessionStartInfoContract startInfo, string endpointPrefix)
        {
            ...
            return await AllocateInternal(startInfo, endpointPrefix, false);
        }
        ...
        /// <param name="sessionInfo">the session info</param>
        /// <returns>the Broker Launcher EPRs, sorted by the preference.</returns>
        async Task<SessionAllocateInfoContract> ISessionLauncher.AllocateDurableV5Async(SessionStartInfoContract startInfo, string endpointPrefix)
        {
            ...
            return await AllocateInternal(startInfo, endpointPrefix, true);
        }
        ...
        /// <returns>the session information.</returns>
        async Task<SessionInfoContract> ISessionLauncher.GetInfoV5Sp1Async(string endpointPrefix, int sessionId, bool useAad)
        {
        ...
        /// <param name="sessionInfo">the session info</param>
        /// <returns>the Broker Launcher EPRs, sorted by the preference.</returns>
        string[] ISessionLauncher.Allocate(SessionStartInfoContract startInfo, string endpointPrefix, out int sessionid, out string serviceVersion, out SessionInfoContract sessionInfo)
        {
            SessionAllocateInfoContract contract = AllocateInternal(startInfo, endpointPrefix, false).GetAwaiter().GetResult();
        ...
        /// <param name="sessionInfo">the session info</param>
        /// <returns>the Broker Launcher EPRs, sorted by the preference.</returns>
        string[] ISessionLauncher.AllocateDurable(SessionStartInfoContract startInfo, string endpointPrefix, out int sessionid, out string serviceVersion, out SessionInfoContract sessionInfo)
        {
            SessionAllocateInfoContract contract = AllocateInternal(startInfo, endpointPrefix, true).GetAwaiter().GetResult();
            sessionid = contract.Id;
            serviceVersion = contract.ServiceVersion?.ToString();
            sessionInfo = contract.SessionInfo;
            return contract.BrokerLauncherEpr;
        ...
        SessionInfoContract GetInfo(string headnode, string endpointPrefix, int sessionId, ISchedulerJob schedulerJob)
        {
            SessionInfoContract sessionInfo = null;
            CheckAccess();
        ...
        /// <param name="headnode">the headnode.</param>
        /// <param name="sessionId">the session id</param>
        async Task ISessionLauncher.TerminateV5Async(int sessionId)
        {
        ...
        async Task<Version[]> ISessionLauncher.GetServiceVersionsAsync(string serviceName)
        {
            using (GetCallerWindowsIdentity().Impersonate())
            {
...
#pragma warning restore 618 // disable obsolete warning for UserPrivilege
                ...
                return await Task.FromResult(GetServiceVersionsInternal(serviceName, true));
            }
        }
        ...
        /// the caller since broker node's machine account is not HpcUser or HpcAdmin.
        /// </remarks>
        async Task<string> ISessionLauncher.GetSOAConfigurationAsync(string key)
        {
        ...
        async Task<Dictionary<string, string>> ISessionLauncher.GetSOAConfigurationsAsync(List<string> keys)
        {
            WindowsImpersonationContext impersonationContext = null;
        ...
        private async Task<SessionAllocateInfoContract> AllocateInternal(SessionStartInfoContract startInfo, string endpointPrefix, bool durable)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    try
                    {
                        ISchedulerCounters counters = scheduler.GetCounters();
...
#region Session Pool
        ...
        private void ConstructSessionPool()
        {
            ...
            using (ISchedulerRowEnumerator rows = this.scheduler.OpenJobEnumerator(props, filters, sorts))
            {
                foreach (PropertyRow row in rows)
                {
                    ...
                    ISchedulerJob job = scheduler.OpenJob(id);
        ...
        private int PickSessionIdFromPool(string serviceNameWithVersion, bool durable, int poolSize, out ISchedulerJob sessionJob)
        {
            ...
            while (true)
            {
                lock (dictionary)
                {
                    ...
                    if (sp.Length + sp.Preparing >= poolSize)
                    {
                        if (sp.Length > 0)
                        {
                            ...
                            TraceHelper.TraceEvent(TraceEventType.Verbose,
                                "[SessionLauncher] .PickSessionIdFromPool: Pick the session {0} in the pool. Next index is {1}", jobid, sp.Index);
                ...
                if (jobid > 0)
                {
                    ...
                    {
                        TraceHelper.TraceEvent(TraceEventType.Error,
                            "[SessionLauncher] .PickSessionIdFromPool: Failed to get the state of job {0}. {1}", jobid, e.ToString());
                    }
                }
                else if (jobid == -1)
                {
                    TraceHelper.TraceEvent(TraceEventType.Verbose,
                                "[SessionLauncher] .PickSessionIdFromPool: wait for the sessions ready for the session pool.");
        ...
        {
            return Task.FromResult<ClusterInfoContract>(this.clusterInfo.Contract);
        }
        ...
        /// </summary>
        [Flags]
        private enum SchedulerConnectState
        {
            /// <summary>
            /// means null.
            /// </summary>
            ...
            /// </summary>
            ConnectionComplete = SchedulerConnectState.ConnectedToSchedulerStore
                                | SchedulerConnectState.StartedBrokerNodesManager
                                | SchedulerConnectState.StartedJobMonitor,
        }
    }
}