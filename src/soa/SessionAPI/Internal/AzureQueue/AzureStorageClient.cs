...
namespace Microsoft.Hpc.Scheduler.Session.Internal
{
    ...
    internal class AzureStorageClient : DisposableObject
    {
        ...
        {
            Message wcfMessage = AzureQueueMessageItem.Deserialize(message.AsBytes);
            ...
            if (wcfMessage.Headers.FindHeader(Constant.MessageHeaderBlob, Constant.HpcHeaderNS) > 0)
            {
                using (wcfMessage)
                {
                    ...
                    return AzureQueueMessageItem.Deserialize(data);
                }
            }
            else
            {
        ...
        public IAsyncResult BeginAddMessage(byte[] messageData, UniqueId messageId, AsyncCallback callback, ReliableQueueClient.ReliableState state)
        {
            ...
            catch (ArgumentException e)
            {
                ...
                SessionBase.TraceSource.TraceEvent(TraceEventType.Warning, 0, "AzureStorageClient, BeginAddMessage, BeginAddMessage failed, {0}", e);
            }
            catch (Exception ex)
            {
                SessionBase.TraceSource.TraceEvent(TraceEventType.Error, 0, "AzureStorageClient, BeginAddMessage, BeginAddMessage failed, {0}", ex);
                throw;
            }

            SessionBase.TraceSource.TraceInformation("AzureStorageClient, BeginAddMessage, Upload message {0} to storage blob.", messageId);
            ...
            message = new CloudQueueMessage(AzureQueueMessageItem.Serialize(referralMessage));
        ...
        protected override void Dispose(bool disposing)
        {
            ...
            if (disposing)
            {
                if (this.ReliableQueue != null)
                {
                    ...
                    {
                        SessionBase.TraceSource.TraceEvent(TraceEventType.Error, 0, "AzureStorageClient, Dispose, Closing ReliableQueue failed, {0}", e);
                    }
        ...
        private void EndDeleteMessage(IAsyncResult asyncResult)
        {
            ...
            try
            {
                ...
                if (this.onAzure)
                {
                    if (messageId != null)
                    {
                        ...
                        {
                            SessionBase.TraceSource.TraceEvent(TraceEventType.Error, 0, "AzureStorageClient, EndDeleteMessage, DeleteBlob failed, {0}", e);
                        }
                    }
                }
            ...
            catch (Exception e)
            {
                ...
                SessionBase.TraceSource.TraceEvent(TraceEventType.Error, 0, "AzureStorageClient, EndDeleteMessage, Error occurs when delete message {0}, {1}", messageIdString, e);
            }
        }
    }
}
