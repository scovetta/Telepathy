...
namespace Microsoft.Hpc
{
    ...
    internal static class Credentials
    {
        ...
        public enum NET_USER_ADD_RETURN_CODE
        {
            ...
            try
            {
                ...
                if (errCode != CredentialNativeMethods.NERR_Success)
                {
                    switch (errCode)
                    {
                        ...
                        default:
                            var errCodeName = Enum.IsDefined(typeof (NET_USER_ADD_RETURN_CODE), errCode) ? $"{(NET_USER_ADD_RETURN_CODE) errCode}({errCode})" : errCode.ToString();
                            throw new Exception($"Error when trying to create local account {username}. Function: NetUserAdd, error code: {errCodeName}, sub-error: {errParam}");
                    }
                }
            }
            finally
        ...
        internal static bool IsUserInSameDomainOrLocalSystem(string userAccount)
        {
            ...
            System.Diagnostics.Trace.TraceInformation($"[{nameof(IsUserInSameDomainOrLocalSystem)}]userAccount:{userAccount}, userDomainName:{Environment.UserDomainName}");

            if (userAccount.Contains("\\"))
            {
                if (userAccount.StartsWith("NT AUTHORITY\\", StringComparison.OrdinalIgnoreCase))
                {
                    return true;  //built in user
                }

                string[] strs = userAccount.Split(new char[] { '\\' }, StringSplitOptions.None);
                bool res = string.Equals(strs[0], Environment.UserDomainName, StringComparison.OrdinalIgnoreCase);
                System.Diagnostics.Trace.TraceInformation($"[{nameof(IsUserInSameDomainOrLocalSystem)}]result:{res}");
                return res;
            }
        ...
        /// <summary>
        /// if the account is email format username@domain, will return domain\\newUsername, 
        /// newUsername is unique local use name to avoid some special charactor in username
        /// </summary>
        internal static string ToDomainFormatUser(string account)
        {
            ...
            string[] strs = account.Split(new char[] { '@' }, StringSplitOptions.None);
            if (strs.Length == 2)
            {
                var newUserName = ToUniqueLocalAccount(strs[0]);
                return string.Format($"{strs[1]}\\{newUserName}");
            }

            throw new ArgumentException($"invalid email account {account}");
        }

        /// <summary>
        /// Verify if userAccount is one of local system accounts: NT AUTHORITY\SYSTEM, NT AUTHROITY\LOCAL SERVICE or NT AUTHORITY\NETWORK SERVICE.
        /// </summary>
        /// <param name="userAccount">user account</param>
        /// <returns></returns>
        internal static bool IsUserInLocalSystem(string userAccount)
        {
            if (string.IsNullOrWhiteSpace(userAccount))
            {
                throw new ArgumentNullException("userAccount");
            ...
            return userAccount.StartsWith("NT AUTHORITY\\", StringComparison.OrdinalIgnoreCase);
        }
        ...
        // This is to make the password has at least one alphabet, one digital and one other chars
        const string PasswordSuffix = "a1;";
        ...
        internal static string GeneratePassword(string baseString)
        {
            SHA256 sha256 = SHA256.Create();
            sha256.ComputeHash(Encoding.Default.GetBytes(baseString));
            byte[] hash = sha256.Hash;
            return Convert.ToBase64String(hash).Substring(0, 16) + PasswordSuffix;
        }
