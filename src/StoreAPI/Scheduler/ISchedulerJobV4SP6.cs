...
namespace Microsoft.Hpc.Scheduler
{
    ...
    [ComVisible(true)]
    [GuidAttribute(ComGuids.GuidIJobV4SP6)]
    public interface ISchedulerJobV4SP6
    {
        ...
        ///   <para>Unless you specify dependencies for the tasks, the tasks you add to a job do not necessarily run in any particular order, except that node preparation and node release tasks are the first and last 
        /// tasks to run on a node, respectively. The HPC Job Scheduler Service tries to run as many tasks in parallel as it can, which may preclude the tasks from running in the order in which you added them.</para> 
        ...
        ///   <para>If the user changed a property value that the template marks as read-only, the property value is silently set to the template's default value. If the user set a property value that is outside the range of the template's constraint on that property, the method fails. Because the 
        ...
        /// True before a task in job that has one of the parametric task types starts running, the method does not show the instances of that task. The task must be running before a call to 
        ...
        ///   <para>If you use the <paramref name="filter" /> parameter to get tasks based on their name, you can only get basic and parametric master tasks that meet the name criteria that the 
        ...
        ///   <para>For those properties that are defined in the new and old template, the default values and template values defined in the new template override the values for the same properties defined in the old template. All property values changed by the user 
        /// remain unchanged. The default values for any properties defined in the old template that are not also defined in the new template are retained. The method does not validate the job's property values against the new template until the scheduler tries to run the job.</para> 
        ///   <para>Job templates are used to set default values, which can be adjusted by subsequent property changes. Therefore, when creating a job, the job template should be applied before any property adjustments, including adding node groups. If property adjustments 
        /// are made prior to applying the job template, default values defined in the job template will not be applied, but required values will still be checked. If the default value is required and not applied, a job template validation error can result.</para> 
        /// </remarks>
        /// <example />
        ...
        /// it needs to be fairly accurate because it is used to allocate resources. If the job exceeds this time, the job is terminated and its state becomes Canceled.</para> 
        ///   <para>The sum of all 
        ...
        ///   <para>The job can run when its minimum resource requirements are met. The scheduler may allocate up to the maximum specified resource limit for the job. The scheduler will allocate more resources to the job or release resources if the 
        ...
        /// </value>
        /// <remarks>
        ///   <para>A node group is typically created to identify a group of nodes with a similar characteristic. For example, nodes that contain a specific software license. You can then use this property to identify the nodes instead of having to specify each node using the 
        /// <see cref="Microsoft.Hpc.Scheduler.ISchedulerJob.RequestedNodes" /> property.</para> 
        ...
        /// not, the scheduler checks if sufficient resources are in nodes 3, 4, 5, and 6, and uses them if they are sufficient. If that also fails, then the job remains queued.</para> 
        ///       </description>
        ///     </item>
        ///   </list>
        ...
        /// <see cref="Microsoft.Hpc.Scheduler.IScheduler.CancelJob(int,string)" /> method, except that the 
        ...
        /// <seealso cref="Microsoft.Hpc.Scheduler.IScheduler.CancelJob(int,string)" />
        void Finish();
        ...
        /// </param>
        /// <remarks>
        ///   <para>If you set or unset an environment variable for a job, that environment variable is also set or unset for each task in the job unless you override that environment variable setting for the task by calling the 
        ...
        /// <seealso cref="Microsoft.Hpc.Scheduler.ISchedulerJob.AddTask(Microsoft.Hpc.Scheduler.ISchedulerTask)" />
        /// <seealso cref="Microsoft.Hpc.Scheduler.ISchedulerJob.CreateTask" />
        /// <seealso cref="Microsoft.Hpc.Scheduler.ISchedulerJob.CancelTask(ITaskId)" />
        ...
        /// </param>
        /// <remarks>
        ///   <para>The HPC Job Scheduler Service only runs the job at the date and time that you specify in the <paramref name="holdUntil" /> parameter if the resources needed 
        /// for the job are available. If the resources needed for the job are not available at that date and time, the job remains queued until the necessary resources become available.</para> 
        ...
        ///           </para>
        ///   <para>To unset the date and time that the HPC Job Scheduler Service should wait until before running the job, which removes the hold on the job and allows it to run, call the 
        ...
        /// </param>
        /// <remarks>
        ///   <para>If you add a node to the list of nodes that should not be used for the job while the job is running on that node, the tasks in the job that are running on the node are canceled and then requeued if the 
        ...
        ///   <para>This method succeeds if and only if it does not generate an exception. This method does not return a value. To verify that the method succeeded, use the 
        ...
        ///   <para>If you specify a node that does not currently belong to the HPC cluster, an exception occurs. If you specify a node that is part of the HPC cluster but is not part of the current list of nodes that should not be used for the job, the 
        ...
        /// </value>
        /// <remarks>
        ///   <para>If your application does not set the value of this property, the HPC Job Scheduler Service calculates the progress based on the percentage of tasks that are complete for 
        /// the job. Once you application sets this property for a job, the HPC Job Scheduler Service does not continue to update this property, and you application must continue to update the property.</para> 
        ...
        /// Information from Applications or Scripts that Run on a Windows HPC Server 2008 R2 Cluster</see> (http://go.microsoft.com/fwlink/?LinkID=177608).</para> 
        /// </example>
        ...
        ///   <para>This property is updated periodically based on the RebalancingInterval cluster parameter. During an adjustment, the broker updates job properties and evaluates the value of the 
        ...
        /// job are available. If the resources needed for the job are not available at that date and time, the job remains queued until the necessary resources become available.</para> 
        ...
        /// </value>
        /// <remarks>
        ///   <para>You cannot add nodes to or remove nodes from the set of nodes that should not be used for the job by adding node names to or removing node names from the 
        ...
        /// the ExcludedNodesLimit parameter of the <see href="http://technet.microsoft.com/library/ff950175.aspx">Set-HpcClusterProperty</see> property. This limit can be modified by the cluster administrator.</para> 
        /// </remarks>
        /// <seealso cref="Microsoft.Hpc.Scheduler.ISchedulerJob.AddExcludedNodes(Microsoft.Hpc.Scheduler.IStringCollection)" />
        /// <seealso cref="Microsoft.Hpc.Scheduler.ISchedulerJob.ClearExcludedNodes" />
        /// <seealso cref="Microsoft.Hpc.Scheduler.ISchedulerJob.RemoveExcludedNodes(Microsoft.Hpc.Scheduler.IStringCollection)" />
        ...
        ///   <para>When you specify an email address for receiving notifications about a job, the notifications are not turned on automatically. You still need to set the 
        ...
        /// <summary>
        /// Finish a task directly. So the task will be never requeued.
        /// </summary>
        ...
        #region V4SP6 methods / properties
